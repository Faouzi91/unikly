...
### Phase B–H — Job Workflow Enhancement (Edit Rules, IN_REVIEW, Proposal Versioning, Cancel, Resubmit)

#### Added — Backend (job-service)

- **Job edit rules engine** (`JobEditRulesEngine`) — evaluates edits against live proposal state and returns an `EditDecision` (allowed, requiresConfirmation, sensitiveFieldsChanged, proposalImpact). Sensitive fields: `budget`, `currency`, `skills`, `description`.
- **`POST /api/v1/jobs/{id}/check-edit`** — dry-run edit eligibility check; returns `EditDecision` without mutating state.
- **`PATCH /api/v1/jobs/{id}?confirmed=true`** — confirmed job update; marks existing SUBMITTED/VIEWED/SHORTLISTED proposals as OUTDATED.
- **`POST /api/v1/jobs/{id}/cancel`** — client-initiated job cancellation; rejects all active proposals and publishes `JobCancelledEvent`.
- **`PUT /api/v1/jobs/{id}/proposals/{pid}/resubmit`** — freelancer resubmit on OUTDATED proposal; updates budget/coverLetter and restores status to SUBMITTED.
- **`IN_REVIEW` job status** — inserted between OPEN and IN_PROGRESS; set when a proposal is accepted.
- **`SUBMITTED`, `VIEWED`, `SHORTLISTED`, `OUTDATED`, `NEEDS_REVIEW` proposal statuses** — richer proposal lifecycle.
- **`job_version` column on `proposals`** — tracks which version of the job a proposal was submitted against.
- **`V3__workflow_enhancements.sql`** Flyway migration — adds `IN_REVIEW` to `job_status` enum, new `proposal_status` values, `job_version` column.
- **`GlobalExceptionHandler`** handlers for `JobNotEditableException`, `DuplicateProposalException`, `EditConfirmationRequiredException` (→ 409), `InvalidProposalStateException` (→ 409).
- **`JobEditWorkflowIntegrationTest`** — 9 integration tests covering the full edit/cancel/resubmit/payment flow.

#### Added — Backend (common)

- **`JobUpdatedEvent`** — includes `List<UUID> affectedFreelancerIds` and `int newVersion`.
- **`JobCancelledEvent`** — includes `List<UUID> affectedFreelancerIds`.

#### Added — Backend (notification-service)

- **`handleJobUpdated`** — idempotent handler; notifies each affected freelancer that their proposal may be outdated.
- **`handleJobCancelled`** — idempotent handler; notifies affected freelancers and the client.

#### Added — Frontend (Angular)

- **`IN_REVIEW` status** support in `JobStatus`, amber badge "Awaiting Payment" in job detail.
- **Edit confirmation flow** in job detail — calls `checkEditEligibility`, shows SweetAlert2 confirm dialog when `requiresConfirmation=true`, then sends `PATCH ?confirmed=true`.
- **Cancel job button** in job detail — SweetAlert2 confirm → `cancelJob()` → redirects to `/jobs`.
- **Version badge** `v{n}` on job detail card.
- **Proposal version warning** — amber badge per proposal card when `proposal.jobVersion < job.version`.
- **Alert banner** for OUTDATED/NEEDS_REVIEW proposals with "Review & Resubmit" CTA.
- **Resubmit flow** via extended `ProposalDialogComponent` (pre-filled budget/coverLetter in resubmit mode).
- **Lifecycle stepper** updated: OPEN → IN_REVIEW → IN_PROGRESS → COMPLETED.

#### Changed — Backend

- `updateJob` collects `affectedFreelancerIds` before bulk proposal status update (prevents missing IDs after OUTDATED bulk-set).
- `cancelJob` and `transitionStatus` (cancel path) collect `affectedFreelancerIds` before bulk REJECTED update.
- `JobService.updateJob` no longer calls `job.setVersion(...)` — Hibernate `@Version` auto-increments.

#### Fixed

- Removed duplicate `JobEditRulesEngine` bean from stale package — was causing `ConflictingBeanDefinitionException` in Spring Boot.
- `JobCancelledEvent` missing `import java.util.List` — caused compilation failure after adding `affectedFreelancerIds`.
- `SearchIntegrationTest` — upgraded Elasticsearch Testcontainer to 9.0.0 to match `elasticsearch-java 9.2.5` client version.
- `UserProfileController.getCurrentProfile` — safe cast `authentication.getPrincipal() instanceof Jwt j ? j : null` to prevent `ClassCastException` in non-JWT test contexts.
- `UserProfileIntegrationTest.testGetMyProfile_notFound_returns404` — corrected to test `GET /{id}` (which returns 404) rather than `GET /me` (which auto-creates profiles).
- `MessagingIntegrationTest.testGetConversations_returnsEmptyPage` — applied `springSecurity()` to MockMvc so `@AuthenticationPrincipal Jwt` is resolved; used fresh token UUID to avoid test-state leak.
