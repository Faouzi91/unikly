...
import com.unikly.userservice.adapter.in.web.mapper.UserProfileMapper;
import com.unikly.userservice.domain.model.UserProfile;
import com.unikly.userservice.domain.model.UserRole;
import com.unikly.userservice.application.port.out.UserProfileRepository;
...
public class UserProfileService {
    ...
    /**
     * Re-publishes UserProfileUpdatedEvent for every FREELANCER profile.
     * This triggers the Search Service to re-index each profile in Elasticsearch.
     * Intended for admin use to backfill the search index.
     *
     * @return the number of profiles queued for re-indexing
     */
    @Transactional
    public int reindexAllFreelancers() {
        var freelancers = profileRepository.findAllByRole(UserRole.FREELANCER);
        for (var profile : freelancers) {
            publishProfileUpdatedEvent(profile);
        }
        log.info("Queued {} freelancer profiles for search re-indexing", freelancers.size());
